...
class Game:
	...
	def __init__(self):
		...
		pygame.display.set_caption('That one monster catching game!')
	...
	def start_battle(self, player_monsters, opponent_monsters, character=False):
		self.battle = Battle(
			player_monsters=player_monsters,
			opponent_monsters=opponent_monsters,
			monster_frames=self.monster_frames,
			bg_surf=self.bg_frames['battle'],
			fonts=self.fonts,
			end_battle=self.end_battle,
			character=character,
			sounds=self.audio
