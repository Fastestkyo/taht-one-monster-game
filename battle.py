...
# Add a new option for running in the BATTLE_CHOICES
BATTLE_CHOICES = {
    'full': {
        'attack': {'icon': 'attack', 'pos': vector(0, 0)},
        'defend': {'icon': 'defend', 'pos': vector(0, 40)},
        'switch': {'icon': 'switch', 'pos': vector(0, 80)},
        'run': {'icon': 'run', 'pos': vector(0, 120)} 
    }
}
...
class Battle:
			...
			if keys[pygame.K_SPACE]:
				...
				if self.selection_mode == 'general':
					...
					if self.indexes['general'] == 3:
						if self.character:  # Trainer battle
							TimedSprite(self.current_monster.rect.center, self.monster_frames['ui']['cross'], self.battle_sprites, 1000)
						else:  # Wild Pokémon battle
	...
	def apply_attack(self, target_sprite, attack, amount):
		...
		if attack_element == 'fire'  and target_element == 'fire' or \
		   attack_element == 'water' and target_element == 'water' or \
		   attack_element == 'plant' and target_element == 'plant':
			amount *= 0.5
